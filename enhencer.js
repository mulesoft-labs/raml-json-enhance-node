...
const arraysToObjects = require('./lib/arrays-objects').arraysToObjects;
const recursiveObjectToArray = require('./lib/arrays-objects').recursiveObjectToArray;
const makeConsistent = require('./lib/consistency-helpers');
const fs = require('fs');
/**
...
class RamlJsonEnhancer {
  ...
  enhance(json) {
    ...
    return this.expandRootTypes(json.types)
    .then((expanded) => {
  ...
  enhanceToFile(json, file) {
    ...
    .then((data) => this.writeFile(JSON.stringify(data, null, 2), file));
  }
   ...
   */
  enhaceJson(ramlObj, parentUrl, allUriParameters) {
    ...
    ramlObj.resources.forEach((resource) => {
      ...
      this.enhaceJson(resource, resource.parentUrl + resource.relativeUri,
